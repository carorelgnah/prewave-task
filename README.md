...
If necessary you can update the [`dev/.local.env`](dev/.local.env) file with your own values.
...
```shell
env $(grep -v '^#' dev/.local.env | xargs) ./gradlew bootRu
```
...
### Automated Tests
Automated tests using Testcontainers are available in the project.